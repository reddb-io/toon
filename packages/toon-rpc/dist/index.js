    ...
    async handleText(text) {
        ...
        for (const entry of entries) {
            ...
            if (response !== undefined)
                responses.push(preflightToonResponse(response, isBatch));
        }
...
function snapshotHandlerError(value) {
    try {
        ...
        if (!code || !('value' in code) || !message || !('value' in message))
            return undefined;
        if (!hasData || !('value' in hasData) || typeof hasData.value !== 'boolean')
            return undefined;
        if (!isHandlerErrorCode(code.value))
            return undefined;
        const source = { code: code.value, message: message.value };
        if (hasData.value) {
...
function isHandlerErrorCode(code) {
...
function preflightToonResponse(response, batch) {
    const snapshot = snapshotResponse(response);
    if (snapshot !== undefined) {
        try {
            encode((batch ? toonBatchProbe(snapshot) : snapshot));
            return snapshot;
        }
        catch {
    ...
    encode((batch ? toonBatchProbe(fallback) : fallback));
    return fallback;
}
function toonBatchProbe(response) {
...
function encodeToonBatch(responses) {
    try {
