...
export function encodeFrame(document: Uint8Array): Uint8Array {
...
export class FrameDecoder {
  ...
  private takeFrame(): Uint8Array | undefined {
    ...
    if (headerEnd === 0) throw this.fail('frame length is empty');
    ...
    if (this.buffer.length <= frameEnd) return undefined;
  ...
  private fail(detail: string): FramingError {
...
function concat(left: Uint8Array, right: Uint8Array): Uint8Array {
  if (left.length === 0) return right instanceof Uint8Array ? new Uint8Array(right) : right;
