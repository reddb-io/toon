...
export function parseRecords(input) {
    return parseStreamInternal(input).flatMap((segment) => segment.rows.map((row) => rowRecord(segment.fields, row, 0)));
}
/**
...
function validateResumeBytes(bytes, cursor) {
    ...
    if (cursor.anchor !== undefined) {
        ...
        if (actual.length !== expectedBytes.length || TEXT_DECODER.decode(actual) !== expected) {
            throw new ToonlCursorInvalidationError('anchor-mismatch', 'TOONL cursor invalidated by anchor mismatch', { byteOffset });
        }
    }
}
...
export function ToonlDecodeStream() {
    ...
    return new WebTransformStream({
        transform(chunk, controller) {
            consumeBufferedLines(state, chunkText(state.textDecoder, chunk, true), controller, enqueueDecodedRecord);
        },
        flush(controller) {
...
export function ToonlToJsonl() {
    ...
    return new WebTransformStream({
        transform(chunk, controller) {
            consumeBufferedLines(state, chunkText(state.textDecoder, chunk, true), controller, enqueueJsonlRecord);
        },
        flush(controller) {
...
function continuationDue(options, rowsSinceContinuation, bytesSinceContinuation) {
    return ((options.continuationEveryRows !== undefined &&
