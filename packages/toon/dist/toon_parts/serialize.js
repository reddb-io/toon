...
function writeField(output, key, value, depth, options) {
    ...
    if (isPlainObject(value)) {
        ...
        if (shape !== undefined) {
            output.push(canonicalKey(key), '{', delimiterPrefix(options.delimiter), shape.fields.map((field) => headerFieldText(field, options.delimiter)).join(options.delimiter), '}:\n');
            for (const [rowKey, rowValue] of Object.entries(value)) {
                writeIndent(output, depth + 1);
                const childOutput = [];
                output.push(canonicalKey(rowKey), ': ', shape.paths
...
function objectShape(records, options, depth) {
    ...
    for (const field of fields) {
        ...
        if (options.primitiveArrayColumns &&
        ...
        if (options.objectArrayColumns && cells.every(({ value }) => Array.isArray(value))) {
            ...
            const children = objectShape(cells.flatMap(({ value }) => value.map((child) => ({ value: child }))), options, depth + 1);
