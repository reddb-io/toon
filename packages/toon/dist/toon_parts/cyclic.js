...
function isCyclicSectionLike(value) {
    return (Object.prototype.hasOwnProperty.call(value, 'order') ||
...
function writeCyclicTable(output, key, fields, rows) {
    output.push('  ', canonicalKey(key), '[', String(rows.length), CYCLIC_TABLE_DELIMITER, ']{', fields.map(canonicalKey).join(CYCLIC_TABLE_DELIMITER), '}:\n');
    for (const row of rows) {
        output.push('    ', fields.map((field) => primitiveText(row[field], CYCLIC_TABLE_DELIMITER)).join(CYCLIC_TABLE_DELIMITER), '\n');
    }
}
