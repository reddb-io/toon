...
import { findUnquoted, parseKey, parseScalar, setKey, splitDelimited } from '../lexical.js';
...
import { atLine, lengthMismatch, parseHeader, parseMapHeader, parseTabularCell } from './parse_headers.js';
    ...
    for (let index = 0; index < lines.length; index += 1) {
        ...
        if (header.fields === undefined && valuePart !== '') {
            ...
            if (actual !== header.len) {
                return truncationReport('array_length_mismatch', line.number, header.len, actual, `declared ${header.len} items but received ${actual}`);
            }
            continue;
        }
        let actual = 0;
        ...
        if (actual < header.len) {
            ...
            return truncationReport(header.fields === undefined ? 'array_length_mismatch' : 'array_length_mismatch', last === undefined ? fallbackLine : last.number, header.len, actual, `declared ${header.len} rows but received ${actual}`);
        }
    }
    const last = lines[lines.length - 1];
    return truncationReport('unterminated_nesting', last === undefined ? fallbackLine : last.number, null, null, 'document ended before the declared nested structure was complete');
}
function detectToonlTruncation(input) {
    ...
    for (const [offset, rawLine] of input.split(/\n/).entries()) {
        ...
        if (line.startsWith('[=') && line.endsWith(']')) {
            ...
            if (declared !== open.actual) {
                return truncationReport('toonl_trailer_count_mismatch', lineNumber, declared, open.actual, `trailer declared ${declared} rows but received ${open.actual}`);
            }
            open = null;
            continue;
        }
    ...
    if (open !== null) {
        return truncationReport('toonl_missing_trailer', open.line, null, open.actual, `stream ended without a trailer after ${open.actual} rows`);
    }
    return completeReport();
}
...
function parseArrayField(header, valuePart, lines, cursor, headerDepth, options) {
    ...
    if (inline !== '') {
        const values = splitDelimited(inline, header.delimiter, headerLine).map((value) => parseScalar(value, headerLine));
...
function parseStructuredRows(len, fields, delimiter, lines, cursor, rowDepth, options, root) {
    const rows = [];
    const childTableFields = inferChildTableFields(len, fields, delimiter, lines, cursor.index, rowDepth, options);
...
function parseStructuredField(field, remainingFields, cells, line, lines, state, childDepth, delimiter, options) {
    ...
    if (field.children !== undefined) {
        ...
        const knownChildTable = state.childTableFields === undefined ? undefined : state.childTableFields.has(field);
        ...
        if (mustBeChildTable) {
            ...
            state.cellIndex += 1;
            return parseStructuredRows(count, field.children, delimiter, lines, { get index() { return state.nextIndex; }, set index(value) { state.nextIndex = value; } }, childDepth, options, false);
        }
...
function inferChildTableFields(len, fields, delimiter, lines, startIndex, rowDepth, options) {
    ...
    for (let mask = 0; mask < combinations; mask += 1) {
        ...
        const result = validateStructuredRowsWithKind(len, fields, childTableFields, delimiter, lines, startIndex, rowDepth, options);
        if (result === undefined) {
            continue;
        }
        if (best === undefined ||
...
function validateStructuredRowsWithKind(len, fields, childTableFields, delimiter, lines, startIndex, rowDepth, options) {
    ...
    try {
        for (let row = 0; row < len; row += 1) {
            ...
            const cells = splitDelimited(line.content, delimiter, line.number);
            const result = validateStructuredRowWithKind(fields, childTableFields, cells, delimiter, lines, index + 1, rowDepth + 1, options);
            if (result === undefined) {
                return undefined;
            }
...
function validateStructuredRowWithKind(fields, childTableFields, cells, delimiter, lines, startIndex, childDepth, options) {
    let cellIndex = 0;
    ...
    for (const field of fields) {
        if (field.fixedLength !== undefined) {
            ...
            if (childTableFields.has(field)) {
                ...
                cellIndex += 1;
                const nestedChildTableFields = inferChildTableFields(count, field.children, delimiter, lines, nextIndex, childDepth, options);
                if (nestedChildTableFields === undefined) {
                    return undefined;
                }
                const result = validateStructuredRowsWithKind(count, field.children, nestedChildTableFields, delimiter, lines, nextIndex, childDepth, options);
                if (result === undefined) {
                    return undefined;
                }
