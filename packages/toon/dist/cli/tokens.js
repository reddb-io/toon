...
export function estimateTokenCount(text) {
    ...
    while (index < characters.length) {
        ...
        if (isJavaScriptWhitespace(first)) {
            ...
            const end = takeSegment(characters, index, (character) => !isJavaScriptWhitespace(character) && !isPunctuation(character));
...
export function formatStatistics(json, toon) {
...
function takeSegment(characters, start, matches) {
    let end = start;
    while (end < characters.length && matches(characters[end]))
        end++;
    return end;
}
function utf16Length(characters, start, end) {
    let length = 0;
    for (let index = start; index < end; index++)
        length += characters[index].length;
    return length;
}
...
function languageCharsPerToken(segment) {
...
const JAVASCRIPT_WHITESPACE_RANGES = [
...
function inRanges(ranges, character) {
    const code = character.codePointAt(0);
    return ranges.some(([start, end]) => code >= start && code <= end);
}
function isGerman(character) {
    return GERMAN.has(character);
}
function isRomance(character) {
    return ROMANCE.has(lowercase(character));
}
function isCentralEuropean(character) {
