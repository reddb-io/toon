...
export type ToonEvent =
  | { type: 'startObject'; line: number }
  | { type: 'endObject'; line: number }
  | { type: 'startArray'; length: number; line: number }
  | { type: 'endArray'; line: number }
  | { type: 'key'; key: string; line: number }
  | { type: 'primitive'; value: JsonPrimitive; line: number }
