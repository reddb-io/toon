...
export async function runCli(argv: readonly string[], io: CliIo): Promise<number> {
  ...
  try {
    ...
    const input: InputSource = !args.input || args.input === '-'
      ? { type: 'stdin' }
...
function resolveDelimiter(value: string): Delimiter {
  if (!value) return DELIMITERS.comma

  const named = (DELIMITERS as Record<string, Delimiter>)[value]
  if (named) return named
  if (value === '\\t') return DELIMITERS.tab
  if (value === DELIMITERS.comma || value === DELIMITERS.tab || value === DELIMITERS.pipe) {
    return value
  }

  throw new CliError(
    `Invalid delimiter ${JSON.stringify(value)}. `
