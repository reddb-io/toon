...
const BY_NAME = new Map(OPTIONS.map((option) => [option.name as string, option]))
const BY_ALIAS = new Map(
  OPTIONS.filter((option) => option.alias).map((option) => [option.alias, option]),
)
...
export function parseCliArgs(argv: readonly string[]): ParsedArgs {
  ...
  if (positionals.length > 0) parsed.input = positionals[0]
  for (const surplus of positionals.slice(1)) unknown.push(JSON.stringify(surplus))
...
function assign(parsed: ParsedArgs, name: keyof ParsedArgs, value: string | boolean): void {
  ;(parsed as unknown as Record<string, unknown>)[name] = value
}
