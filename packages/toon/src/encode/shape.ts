...
export function tabularFields(rows: Record<string, any>[]): FieldNode[] | undefined {
...
export function keyedFields(value: Record<string, any>): FieldNode[] | undefined {
...
export function collectLeaves(value: Record<string, any>, fields: FieldNode[]): any[] {
  const leaves = []
  for (const field of fields) {
    if (field.children === undefined) leaves.push(value[field.name])
    else leaves.push(...collectLeaves(value[field.name], field.children))
  }
  return leaves
}
