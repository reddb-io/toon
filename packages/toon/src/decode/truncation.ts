...
export function detectTruncation(
  ...
  if (format === 'toonl') return detectToonlTruncation(input)
  if (format !== 'toon') throw new TypeError('detectTruncation format must be "toon" or "toonl"')
...
function arrayMismatch(error: unknown): TruncationReport | undefined {
  if (!(error instanceof ToonDecodeError || error instanceof ToonError)) return undefined
  ...
  return mismatchReport(
    ...
    Number(match[1]),
    Number(match[3]),
    match[2] === 'inline-form values' ? 'items' : 'rows',
  )
}
...
function detectToonlTruncation(input: string): TruncationReport {
  let open: { line: number, actual: number } | undefined
    ...
    if (line.startsWith('[=') && line.endsWith(']')) {
      ...
      if (declared !== open.actual) {
        return truncatedReport(
          ...
          declared,
          open.actual,
          `trailer declared ${declared ?? 'invalid'} rows but received ${open.actual}`,
        )
      }
      open = undefined
...
function completeReport(): TruncationReport {
  return {
    complete: true,
    kind: 'complete',
    line: null,
