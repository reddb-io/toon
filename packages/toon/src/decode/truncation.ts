...
export function detectTruncation(
  ...
  if (format === 'toonl') return detectLegacyTruncation(input, options)
...
function findArrayLengthMismatch(
  input: string,
  options: DecodeStreamOptions,
): TruncationReport | undefined {
  ...
  for (const [index, line] of lines.entries()) {
    ...
    for (const nested of lines.slice(index + 1)) {
      if (nested.depth <= line.depth) break
      if (nested.depth === rowDepth) actual += 1
    }
    if (actual < header.declared) {
      return mismatchReport(
        lines.at(-1)?.number ?? line.number,
        header.declared,
        actual,
        'rows',
      )
...
function arrayHeader(line: ContentLine) {
  let open: number
  try {
  ...
  if (segment.endsWith('|') || segment.endsWith('\t')) {
    delimiter = segment.at(-1) as string
...
function contentLines(input: string, indentSize: number): ContentLine[] {
  return input
    .split(/\n/)
