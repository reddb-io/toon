...
interface RowState {
  ...
  nextIndex: number
  flatWidth: number
  childTableFields: Set<ExtensionFieldNode> | undefined
}

/** Decode a buffered tabular span and expose only JSON-semantic events. */
export function* emitExtensionRows(
  fields: ExtensionFieldNode[],
  lines: ExtensionLine[],
  length: number,
  delimiter: string,
  rowDepth: number,
  headerLine: number,
  options: ExtensionDecodeOptions,
): Generator<ToonEvent> {
  const cursor = { index: 0 }
  yield* emitStructuredRows(
    length,
    fields,
    delimiter,
    lines,
...
function* emitStructuredRows(
  ...
  while (rows < length) {
    ...
    if (state.cellIndex !== cells.length) throw toonError(line.number, 'array row length mismatch')
    cursor.index = state.nextIndex
    rows++
  }

  if (rows !== length) throw lengthMismatch(lines, cursor.index, fallbackLine)
...
function* emitStructuredField(
  field: ExtensionFieldNode,
  remainingFields: ExtensionFieldNode[],
  cells: string[],
  line: ExtensionLine,
  lines: ExtensionLine[],
  state: RowState,
  ...
  if (field.children !== undefined) {
    ...
    const childTable = knownChildTable ?? (
      count !== undefined &&
      (hasChildRows || (
...
function* emitFixedList(
  field: ExtensionFieldNode,
  cells: string[],
  state: RowState,
  line: number,
): Generator<ToonEvent> {
  const length = field.fixedLength as number
  if (state.cellIndex + length > cells.length) throw toonError(line, 'array row length mismatch')
...
function takeCell(cells: string[], state: RowState, line: number): string {
  const cell = cells[state.cellIndex]
  if (cell === undefined) throw toonError(line, 'array row length mismatch')
  state.cellIndex++
  return cell
}

function inferChildTableFields(
  length: number,
  fields: ExtensionFieldNode[],
  delimiter: string,
  lines: ExtensionLine[],
...
function validateRows(
  length: number,
  fields: ExtensionFieldNode[],
  childTableFields: Set<ExtensionFieldNode>,
  delimiter: string,
  lines: ExtensionLine[],
  startIndex: number,
  rowDepth: number,
  options: ExtensionDecodeOptions,
): { nextIndex: number, consumed: number } | undefined {
  ...
  try {
    for (let row = 0; row < length; row++) {
      ...
      if (
        line === undefined || line.depth !== rowDepth ||
      ...
      const result = validateRow(
        fields,
        childTableFields,
        splitDelimited(line.content, delimiter, line.number),
        delimiter,
        lines,
        index + 1,
        rowDepth + 1,
      ...
      if (result === undefined || lines[result.nextIndex]?.depth > rowDepth) return undefined
  ...
  for (const field of fields) {
    if (field.fixedLength !== undefined) {
      ...
      if (childTableFields.has(field)) {
        ...
        cellIndex++
        const nested = inferChildTableFields(
          count,
          field.children,
          delimiter,
          lines,
    ...
    if (cellIndex > cells.length) return undefined
  }
  return cellIndex === cells.length ? { nextIndex, consumed } : undefined
}

function parseChildCount(value: string | undefined): number | undefined {
  return value !== undefined && /^(0|[1-9][0-9]*)$/.test(value) ? Number(value) : undefined
}

function leafWidth(fields: ExtensionFieldNode[]): number {
  return fields.reduce((total, field) => total + fieldWidth(field), 0)
}

function fieldWidth(field: ExtensionFieldNode): number {
  if (field.fixedLength !== undefined) return field.fixedLength
  if (field.children !== undefined) return leafWidth(field.children)
  return 1
}

function minimumRowWidth(fields: ExtensionFieldNode[]): number {
  return fields.reduce(
    (total, field) => total + (field.children === undefined ? fieldWidth(field) : 1),
    0,
  )
}

function isTabularRow(content: string, delimiter: string, line: number): boolean {
...
function lengthMismatch(lines: ExtensionLine[], index: number, fallbackLine: number): Error {
