...
import type { JsonRpcError, JsonRpcRequest, JsonRpcResponse, JsonValue } from './jsonrpc.js';
import {
...
import type { McpService, ServerCapabilities } from './types.js';
...
const ERROR_CODES: Record<McpError['kind'], number> = {
...
export class McpDispatcher {
  ...
  readonly #legacyInitialize: boolean;
  ...
  async handleLine(line: string): Promise<string | null> {
    ...
    try {
      ...
      return toLine(
        failure(null, rpcError(PARSE_ERROR, `Parse error: ${(e as Error).message}`))
      );
    }
  ...
  async handleMessage(message: unknown): Promise<JsonRpcResponse | null> {
    ...
    const id = (raw.id ?? null) as JsonValue;
    ...
    const request: JsonRpcRequest = {
      jsonrpc: '2.0',
      method: raw.method,
      params: raw.params as JsonValue | undefined,
    };
    ...
    if (versionError) return failure(id, versionError);
    ...
    try {
      ...
      return failure(id, rpcError(INTERNAL_ERROR, `Internal error: ${(e as Error).message}`));
    }
  }
   ...
   */
  #checkProtocolVersion(request: JsonRpcRequest): JsonRpcError | null {
    ...
    if (supported.includes(requested)) return null;
  ...
  async #capabilities(): Promise<ServerCapabilities> {
    if (this.#service.capabilities) return this.#service.capabilities();

    const caps: ServerCapabilities = {};
    if ((await this.#listTools()).length > 0) caps.tools = { listChanged: false };
    if ((await this.#listResources()).length > 0) caps.resources = {};
    if ((await this.#listPrompts()).length > 0) caps.prompts = {};
    return caps;
  }
  ...
  async discover(): Promise<JsonValue> {
  ...
  async #route(method: string, request: JsonRpcRequest): Promise<JsonValue> {
  ...
  async #initialize(): Promise<JsonValue> {
    if (!this.#legacyInitialize) {
      ...
      throw new McpError(
        'method_not_found',
        `Method not found: initialize. This server speaks MCP ${MCP_PROTOCOL_VERSION}, which ` +
          `replaces the initialize handshake with per-request _meta. Call server/discover instead.`,
  ...
  async #callTool(params: Record<string, unknown>): Promise<JsonValue> {
  ...
  async #readResource(params: Record<string, unknown>): Promise<JsonValue> {
    ...
    // missing resource.
    if (contents.length === 0) throw McpError.resourceNotFound(uri);

    return omitUndefined({ resultType: 'complete', contents });
  }

  async #getPrompt(params: Record<string, unknown>): Promise<JsonValue> {
    const name = requireString(params, 'name');
    if (!this.#service.getPrompt) throw McpError.promptNotFound(name);
    return omitUndefined(await this.#service.getPrompt(name, params.arguments as JsonValue));
  }
}
 ...
 */
function omitUndefined(value: unknown): JsonValue {
