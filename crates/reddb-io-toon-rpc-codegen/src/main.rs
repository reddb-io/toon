...
fn main() {
    ...
    let lang = if args.len() >= 3 { &args[2] } else { "--both" };
...
mod rust {
    ...
    pub fn generate(service: &Service) -> String {
        ...
        out.push_str("use reddb_io_toon_rpc::{RpcContext, RpcResult, Dispatcher};\n\n");
    ...
    fn generate_param_extraction(method: &Method) -> String {
        let mut out = String::new();
        out.push_str("        let ctx = reddb_io_toon_rpc::RpcContext::new();\n");
...
mod ts {
    ...
    fn generate_server(service: &Service) -> String {
        let mut out = String::new();
        out.push_str(&format!("export interface {}Impl {{\n", service.name));
        for method in &service.methods {
            let params: Vec<String> = method
                .params
                .iter()
