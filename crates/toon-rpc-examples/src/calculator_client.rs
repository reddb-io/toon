...
use toon_rpc::{ClientTransport, Id, Params, Request, RpcError, TOONRPC_VERSION, from_wire, to_wire};
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let msg = toon_rpc::protocol::Message::Single(toon_rpc::protocol::Call::Request(request));
    ...
    match parsed {
        toon_rpc::protocol::Message::SingleResponse(resp) => match (resp.result, resp.error) {
            (Some(result), None) => {
