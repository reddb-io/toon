...
use toon_rpc::{Dispatcher, Params};

fn extract_numbers(params: &Params) -> Result<Vec<f64>, toon_rpc::RpcError> {
    match params {
        Params::ByPosition(values) => values
            ...
            .map(|v| match v {
                serde_json::Value::Number(n) => n
                    .as_f64()
                    .ok_or_else(|| toon_rpc::RpcError::InvalidParams("not a number".to_string())),
                _ => Err(toon_rpc::RpcError::InvalidParams("expected numbers".to_string())),
            })
            .collect(),
        Params::ByName(_) => Err(toon_rpc::RpcError::InvalidParams(
...
fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    dispatcher.register("divide", |params, _id| {
        let nums = extract_numbers(&params)?;
        if nums[1] == 0.0 {
            Err(toon_rpc::RpcError::InvalidParams(
