...
pub fn response_from_wire(raw: &[u8]) -> Result<Response, RpcError> {
    response_from_toon(&decode_wire_value(raw)?)
}

/// Validate one already-decoded response document, such as a batch entry.
pub(crate) fn response_from_toon(toon_value: &ToonValue) -> Result<Response, RpcError> {
