...
fn wire_efficiency_corpora_assert_encoded_byte_sizes_for_rust() {
    ...
    for test_case in cases {
        ...
        let toon_tab = toon_value.to_legacy_toon_with_options(LegacyEncodeOptions {
            delimiter: '\t',
...
fn cyclic_discriminated_array_encoding_is_opt_in_and_pins_the_frozen_wire_for_rust() {
    ...
    let encoded = value.to_legacy_toon_with_options(LegacyEncodeOptions {
...
fn object_array_column_encoding_is_opt_in_and_falls_back_losslessly_for_rust() {
    ...
    assert_ne!(
        encoded,
        value.to_legacy_toon_with_options(LegacyEncodeOptions {
            delimiter: '|',
            ..LegacyEncodeOptions::default()
        })
    );
    ...
    let matrix_encoded = matrix_value.to_legacy_toon_with_options(LegacyEncodeOptions {
        object_array_columns: true,
        delimiter: '|',
        ..LegacyEncodeOptions::default()
    });
...
fn ext_options() -> LegacyEncodeOptions {
    LegacyEncodeOptions {
...
fn encode_options(options: &Json) -> LegacyEncodeOptions {
    LegacyEncodeOptions {
