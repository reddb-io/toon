...
| TS-2 | `EncodeOptions.indent` and `DecodeStreamOptions.indent` fallback to `indentSize` | **genuinely load-bearing** | Both spellings and precedence are part of the pinned upstream v4.1.1 option inventory in `docs/options-parity-v4.1.1.md`; shared conformance adapters and the indent-edge regression test exercise both names. Retain until the upstream contract drops the deprecated spelling; do not mistake deprecation alone for absence of a consumer. |
