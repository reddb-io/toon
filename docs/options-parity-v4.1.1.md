...
All 15 pinned entries are classified: 5 identical, 1 fixed-here, and 9
ledgered divergences. Four ledgered rows are deliberate CLI adaptations or
diagnostic presentation differences; the other five rows form one actionable
gap cluster with a follow-up:
...
The `cli.stats` fix includes stdin/file coverage, a stable tokenx 1.3.0
estimator declaration, byte-for-byte stdout assertions, and exact statistics
diagnostics. The documentation contract test prevents a pinned option or issue
reference from silently disappearing from the inventory.
