...
> **Design history — absorbed by the official spec.** The mechanism described here was adopted into the official TOON specification at **v4.0** (upstream RFC [spec#57](https://github.com/toon-format/spec/issues/57)). The official syntax and semantics now govern; this document is retained as design history and carries no independent normative weight. See [ADR 0005](../../.red/adr/0005-rebase-on-spec-v4-1-with-event-based-decoders.md) and the [migration notes](../migration-v4.md).

**Stage:** 4 — graduated
**Status:** absorbed into official TOON spec v4.0 (was Extension 2 in the reddb flavor); design history.
