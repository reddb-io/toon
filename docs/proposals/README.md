...
- **Surviving reddb-io opt-in extensions.** The rest remain live reddb-io
  extensions re-expressed on the v4.1 base. Primitive/object array wires decode
  by default and fail closed; cyclic reconstruction is opt-in; all three encode
  only on request. Their behavior is defined normatively in
...
| **3** | Implemented opt-in | Shipped behind explicit API options with decode, literal-fallback, and fail-closed behavior documented per feature. |
