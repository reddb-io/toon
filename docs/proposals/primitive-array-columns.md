...
- Empty arrays encode as an empty cell. Null list cells are **not** eligible; use
  ordinary TOON v4.1 fallback if the field itself can be null.
...
and heterogeneous shapes fall back to ordinary TOON v4.1. The encoder MUST NOT
...
falls back to ordinary TOON v4.1 for the whole table rather than emit a lossy
cell.
...
standard TOON v4.1 when eligibility fails.
...
labels" shape while keeping every ineligible case in ordinary v4.1. The **one
...
expanded v4.1 arrays, and it was deliberately accepted because the token win is
